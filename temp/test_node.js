...
function addHelperFunctions() {
  if (!vadModule.getValue) {
    vadModule.getValue = function (ptr, type) {
      switch (type) {
        case "i32":
          return vadModule.HEAP32[ptr >> 2];
        case "float":
          return vadModule.HEAPF32[ptr >> 2];
        default:
...
function generateTestAudio(durationMs = 5000) {
  ...
  console.log(
    `Generating ${totalSamples} samples for ${durationMs}ms audio...`
  );
...
function printResults(processingTime, totalAudioTime, outFlags, frameNum) {
  ...
  console.log(`\n=== Processing Results ===`);
  console.log(
    `Time: ${processingTime}ms, Audio: ${totalAudioTime.toFixed(
      2
    )}ms, RTF: ${rtf.toFixed(6)}`
  );
...
async function testWithArray() {
  ...
  console.log(
    `Audio info: ${byteNum} bytes, ${frameNum} frames, ${totalAudioTime.toFixed(
      2
    )}ms`
  );
  ...
  const processingTime = await processAudio(
    inputBuf,
    frameNum,
    outProbs,
    outFlags
...
async function testWithWAV(inputFile, outputFile) {
  ...
  try {
    ...
    console.log(
      `WAV Format: ${wavInfo.channels} channel(s), ${wavInfo.sampleRate}Hz, ${wavInfo.bitsPerSample}-bit`
    );
    console.log(
      `Total samples: ${wavInfo.totalSamples}, samples per channel: ${wavInfo.samplesPerChannel}`
    );

    // Validate format requirements
    if (wavInfo.sampleRate !== 16000) {
      console.warn(
        `Warning: Sample rate is ${wavInfo.sampleRate}Hz, expected 16000Hz`
      );
    }

    if (wavInfo.channels !== 1) {
      console.warn(
        `Warning: ${wavInfo.channels} channels detected, only first channel will be used`
      );
    }
    ...
    const inputBuf = new Int16Array(
      audioBuffer.buffer.slice(audioBuffer.byteOffset)
    );
    ...
    console.log(
      `Audio info: ${
        audioBuffer.length
      } bytes, ${sampleNum} samples, ${frameNum} frames, ${totalAudioTime.toFixed(
        2
      )}ms`
    );
    ...
    if (wavInfo.channels > 1) {
      ...
      processedInput = new Int16Array(
        Math.floor(inputBuf.length / wavInfo.channels)
      );
...
async function runBenchmark() {
  ...
  for (const numFrames of testCases) {
    ...
    console.log(
      `${numFrames} frames: ${totalTime}ms total, ${avgTime.toFixed(
        3
      )}ms/frame, RTF: ${rtf.toFixed(3)}`
    );
...
async function loadModule() {
  try {
    ...
    vadModule = await createVADModule({
      ...
      locateFile: (filePath) =>
        filePath.endsWith(".wasm") ? WASM_BINARY_FILE : filePath,
      noInitialRun: false,
      noExitRuntime: true,
    });

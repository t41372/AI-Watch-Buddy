...
export const ChromeBrowser = ({ 
  ...
  }, [receivedActions, addActions, clearReceivedActions]);
  ...
  const handleCurrentTimeChange = useCallback((time: number) => {
    ...
    if (actionsToExecute.length > 0) {
      console.log(`Found ${actionsToExecute.length} actions to execute at time ${time}s`);
      
      // Remove actions from queue immediately to prevent re-execution
      const actionIds = actionsToExecute.map(action => action.id);
      removeActions(actionIds);
      
      // Execute the actions
      executeActions(actionsToExecute);
    }
  }, [checkThreshold, getNextActions, removeActions, executeActions]);

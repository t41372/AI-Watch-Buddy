...
class TTSBase64Manager:
    ...
    def process_text_batch(self, texts: list, voice_id: str = None) -> dict:
        ...
        for i, text in enumerate(texts, 1):
            ...
            if result["success"]:
                successful_count += 1
                print(f"✅ 成功 - 音频长度: {result['audio_length']} 字符 ({result['audio_size_bytes']} bytes)")
            else:
...
def test_simple_base64_tts():
    ...
    if result["success"]:
        print(f"\n🎉 批量处理完成！")
        print(f"📊 成功生成: {result['successful_generations']}/{result['total_texts']}")
...
def test_video_reaction_base64():
    ...
    if result["success"]:
        # 合并时间戳信息
        enhanced_results = []
        for i, (reaction, tts_result) in enumerate(zip(mock_video_reactions, result["results"])):
            enhanced_result = {
                **tts_result,
        ...
        for reaction in enhanced_results[:3]:
            if reaction["success"]:
                ...
                print(f"           音频: {reaction['audio_size_bytes']} bytes -> {reaction['audio_length']} chars (base64)")
            else:
...
"""
    ...
    try:
        ...
        # 4. 提取文本并批量生成base64音频
        texts = [action.get('text', '').strip() for action in speak_actions if action.get('text', '').strip()]
        ...
        if batch_result["success"]:
            # 5. 合并时间戳和其他信息
            enhanced_results = []
            for i, (action, tts_result) in enumerate(zip(speak_actions, batch_result["results"])):
                enhanced_result = {
                    **tts_result,
            ...
            print(f"🎵 成功生成: {batch_result['successful_generations']}/{batch_result['total_texts']} 个base64音频")
            ...
            for i, audio_data in enumerate(enhanced_results[:3]):
                if audio_data["success"]:
                    ...
                    print(f"     音频: {audio_data['audio_size_bytes']} bytes -> {audio_data['audio_length']} chars (base64)")
                else:
    ...
    try:
        ...
        if voices:
            ...
            for i, voice in enumerate(voices[:5]):
                print(f"{i+1}. {voice['name']} ({voice['voice_id']}) - {voice['category']}")

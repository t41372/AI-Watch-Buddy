...
def invoke_gemini_vids(
    ...
    Args:
        ...
        api_key (str | None): Gemini API密钥，未提供则从环境变量读取
    ...
    if not allowed_file(video_path):
        return {
            "error": "不支持的文件格式，支持的格式: " + ", ".join(ALLOWED_EXTENSIONS)
        }
...
if __name__ == "__main__":
    ...
    result = invoke_gemini_vids(
        video_path="video_cache/【官方 MV】Never Gonna Give You Up - Rick Astley.mp4",
        system_prompt="请分析视频内容",
        user_prompt="请生成 action list",
    )
